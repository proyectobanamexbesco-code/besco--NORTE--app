...
correo_destino = mapeo_correos.get(oficina, "gerardo.mendez@besco.mx")

# --- CAMPO INFORMATIVO SIN OPCIÓN A EDITAR ---
st.text_input("Responsable que recibirá el reporte (Automático)", value=correo_destino, disabled=True)
...
if st.button("🚀 Generar Reporte Final Múltiple", type="primary"):
    ...
    if "EMAIL_SENDER" in st.secrets:
        if enviar_correo(pdf_bytes, cliente, folio, sucursal, oficina, nombre_pdf, correos_adicionales, f_ejec_str, correo_destino):
            st.success(f"✅ ¡Reporte Enviado a {correo_destino}!")
        else:
